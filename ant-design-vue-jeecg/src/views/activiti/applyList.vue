...
<script>
  ...
  import { deleteAction, getAction, postFormAction } from '@/api/manage'
  ...
  export default {
    ...
    methods: {
      ...
      edit(r,isView) {
        ...
        let formComponent = this.getFormComponent(r.routeName);
      ...
      remove(r) {
        if (r.tableType === '2') {
          deleteAction(r.otherInfo, {id: r.tableId}).then((res) => {
            if (res.success) {
      ...
      chooseProcess(v) {
        ...
        let formComponent = this.getFormComponent(v.routeName);
        this.lcModa.formComponent = () => import(`@/${formComponent.component}`);
